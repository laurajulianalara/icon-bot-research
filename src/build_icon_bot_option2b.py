...
OUT = Path("src/icon_bot_option2b.pine")
...
// THE ICON — OPTION 2B — FINAL STRATEGY\n// ONLY strategy change from frozen 2A: six/day cap counts FINAL valid entries, not V15 survivors.
...
f_eval_candidate(int finalN, float hiRun, float loRun, float pStop, bool pLong, string pSess, bool pOrder) =>
    int finalTradesTodayL = finalN
    float runHiL = hiRun
    float runLoL = loRun
    ...
    [finalTradesTodayL, runHiL, runLoL, pendingStopL, pendingLongL, pendingSessL, orderPendingL]
...
[_finalN, _runHi, _runLo, _pStop, _pLong, _pSess, _pOrder] = f_eval_candidate(finalTradesToday, runHi, runLo, pendingStop, pendingLong, pendingSess, orderPending)
finalTradesToday := _finalN
runHi := _runHi
runLo := _runLo
pendingStop := _pStop
pendingLong := _pLong
...
if barstate.islast and timeframe.in_seconds() != 180
    runtime.error("The Icon Option 2B must run on the 3-minute chart.")
'''
