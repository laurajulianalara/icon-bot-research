...
OUT = Path("src/icon_sep11_diagnostic.pine")
...
var array<float> sep11RunHi = array.new_float()
var array<float> sep11RunLo = array.new_float()
var array<float> sep11BarHi = array.new_float()
var array<float> sep11BarLo = array.new_float()
...
f_candidate_audit(int t, int d) =>
    ...
    if found >= 0
        ...
        if array.size(extraCandTimes) < 12
            ...
            array.push(sep11RunHi, na)
            array.push(sep11RunLo, na)
            array.push(sep11BarHi, na)
            array.push(sep11BarLo, na)
...
new = """                if exists
                    ...
                    if time >= AUDIT_START and time < AUDIT_END
                        ...
                        int exBefore = array.size(extraCandTimes)
                        if exBefore > 0 and exBefore <= 12 and array.get(extraCandTimes, exBefore - 1) == time and array.get(extraCandDirs, exBefore - 1) == diagD
                            array.set(sep11RunHi, exBefore - 1, runHiL)
                            array.set(sep11RunLo, exBefore - 1, runLoL)
                            array.set(sep11BarHi, exBefore - 1, high)
                            array.set(sep11BarLo, exBefore - 1, low)
                    float sg = isLong ? 1.0 : -1.0
"""
if old not in pine:
...
sep11_tail = r"""
var table sep11Table = table.new(position.middle_right, 6, 13, border_width = 1)
if barstate.islast
    table.cell(sep11Table, 0, 0, "FIRST PINE-ONLY CANDIDATES")
    table.cell(sep11Table, 1, 0, "Time")
    table.cell(sep11Table, 2, 0, "Dir")
    table.cell(sep11Table, 3, 0, "Prior Run")
    table.cell(sep11Table, 4, 0, "Bar Extreme")
    table.cell(sep11Table, 5, 0, "Difference")
    int n = math.min(array.size(extraCandTimes), 10)
    if n > 0
        for jj = 0 to n - 1
            int tt = array.get(extraCandTimes, jj)
            int dd = array.get(extraCandDirs, jj)
            float prior = dd == 1 ? array.get(sep11RunLo, jj) : array.get(sep11RunHi, jj)
            float ext = dd == 1 ? array.get(sep11BarLo, jj) : array.get(sep11BarHi, jj)
            float dif = dd == 1 ? prior - ext : ext - prior
            table.cell(sep11Table, 0, jj + 1, str.tostring(jj + 1))
            table.cell(sep11Table, 1, jj + 1, str.format_time(tt, "MM-dd HH:mm", "America/New_York"))
            table.cell(sep11Table, 2, jj + 1, dd == 1 ? "LONG" : "SHORT")
            table.cell(sep11Table, 3, jj + 1, str.tostring(prior))
            table.cell(sep11Table, 4, jj + 1, str.tostring(ext))
            table.cell(sep11Table, 5, jj + 1, str.tostring(dif))
"""
pine += sep11_tail
...
print("CREATED", OUT)
print("VERSION: SEP11 CANDIDATE STATE DIAGNOSTIC")
