...
FEATURES_OUT = "data/v4_causal_reversal_features.parquet"
RESULTS_OUT = "data/strategy_results_v4_causal.csv"
...
def simulate(c, conf, entry_type, rr, stop_buffer):
    ...
    return dict(fill_time=ft,exit_time=xt,outcome=outcome, signal_available_time=signal_time,
...
for n,(_,c) in enumerate(cand.iterrows(),1):
    ...
    for mb in MAX_CONFIRM_BARS:
        ...
        for entry,rr,sb in product(ENTRY_TYPES,RR_VALUES,STOP_BUFFERS):
            ...
            diag_sim_success += 1
            rows.append({**tr,
                "candidate_time":c.time_ny,"confirm_time":conf["confirm_time"],"cisd_bars":conf["cisd_bars"],"session":c.session,"direction":c.direction,
