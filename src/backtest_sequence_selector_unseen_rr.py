...
print("=== 0-3M SEQUENCE — UNSEEN SEQUENTIAL RR BACKTEST ===")
...
# Decision uses completed minute +3; entry is next minute open (+4).
for top in [20,10,5,3]:
    ...
    for rr in range(1,7):
        ...
        for _,x in sig.iterrows():
            i=idx.get(x[tc])
            if i is None or i+4>=len(one): miss+=1; continue
            j=i+4
            if j<=busy: continue
            direction=str(x[dc]).upper()
            entry=float(one.iloc[j].open)
            # stop remains original candidate extreme; known before decision.
...
print("\nSTRICT CAUSAL TIMING: observe through minute +3, ENTER at minute +4 open. No future candles used to decide.")
