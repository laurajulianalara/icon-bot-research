import pandas as pd
import numpy as np
TODAY="data/mnq_sep21_2026_1m.parquet"; HIST="data/mnq_continuous_1m.parquet"
DATE=pd.Timestamp("2026-09-21").date(); RTH=0.576132; WTH=0.183258
...
g=three[(three.time_ny.dt.date==DATE)&(three.time_ny.dt.hour>=2)&(three.time_ny.dt.hour<5)].copy().reset_index(drop=True)
cands=[]; rh=rl=None
for _,r in g.iterrows():
    if rh is None: rh=float(r.high); rl=float(r.low); continue
    rng=float(r.high-r.low)
    if r.low<rl: cands.append(dict(time_ny=r.time_ny,direction="LONG",ticker=r.ticker,extreme=float(r.low),wick_percent=float(r.lower_wick/rng) if rng>0 else 0))
    if r.high>rh: cands.append(dict(time_ny=r.time_ny,direction="SHORT",ticker=r.ticker,extreme=float(r.high),wick_percent=float(r.upper_wick/rng) if rng>0 else 0))
...
cand["next_same_extreme_time"]=cand.groupby("direction").time_ny.shift(-1)
...
for _,c in cand.iterrows():
    ...
    for q in range(j,min(j+241,len(one))):
        ...
        if sh: break
        maxr=max(maxr,fav)
    rows.append(dict(entry_time=signal,direction=c.direction,entry=entry,stop=stop,risk_points=risk,max_rr=maxr))
r=pd.DataFrame(rows)
print("\n=== OPTION 2A — SEPT 21 LONDON ===")
if r.empty: print("QUALIFYING TRADES: 0"); raise SystemExit
r["outcome_4r"]=np.where(r.max_rr>=4,"WIN","LOSS")
print("Trades:",len(r))
print(r[["entry_time","direction","entry","stop","risk_points","outcome_4r","max_rr"]].round(2).to_string(index=False))
print("\nRR HIT RATES")
for rr in range(1,7):
...
r.to_csv("data/sep21_london_option2a.csv",index=False)
