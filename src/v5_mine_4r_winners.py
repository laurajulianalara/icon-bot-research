...
cand=cand.sort_values("time_ny").drop_duplicates(["time_ny","session","direction"])
cm={(r.time_ny,r.session,r.direction):r for _,r in cand.iterrows()}
...
for n,r in d.iterrows():
    ...
    c=cm.get((r.candidate_time,r.session,r.direction))
    ...
    rej=((b.close-b.low)/rng if r.direction=="LONG" else (b.high-b.close)/rng)
    # Prior structural reactions: how many prior 30m bars came within .25 ATR of extreme
    ex=float(c.extreme)
    touches=((pre.low-ex).abs()<=.25*a).sum() if r.direction=="LONG" else ((pre.high-ex).abs()<=.25*a).sum()
    ...
    if len(earlier):
        oldext=float(earlier.low.min() if r.direction=="LONG" else earlier.high.max())
        progress=abs(ex-oldext)/a
    else: progress=np.nan
    rows.append(dict(candidate_time=r.candidate_time,fill_time=r.fill_time,session=r.session,direction=r.direction,outcome=r.outcome,
